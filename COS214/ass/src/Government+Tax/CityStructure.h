...
private:
  std::string Name;
  std::vector<std::unique_ptr<CityBlock>> blocks;
...
public:
  ...
  CityStructure(std::string name);
    ...
    /**
    * @brief Destructor to free allocated memory for CityBlocks.
    */
  ~CityStructure() = default;
  ...
  void accept(TaxAndBudgetVisitor *visitor);
     ...
     */
  void addBlock(std::unique_ptr<CityBlock> block);
