...
public class ExecutionOrderChecker {
    public static List<List<MethodCall>> findPossibleOrders(List<MethodCall> operations) {
        ...
        backtrack(new ArrayList<>(), new HashMap<>(), operations, result);
        return result;
    }

    private static void backtrack(List<MethodCall> current, Map<String, Integer> lastOperationIndex, List<MethodCall> remaining, List<List<MethodCall>> result) {
        ...
        for (int i = 0; i < remaining.size(); i++) {
            ...
            // Ensure thread order is maintained
            ...
            if (isValid(newCurrent)) {
                backtrack(newCurrent, newLastOperationIndex, newRemaining, result);
            }
        }
    }
