...
class Node {
    ...
    public void assignLabels()
    {
      if(isLeaf())
      {
        this.addLabel(Label.FINAL);
        return;
      }
      for(Node children : children)
      {
        children.assignLabels();
      }
      if(this.children.size() == 1)
      {
        this.addLabel(Label.UNIVALENT);
        this.value = this.children.get(0).value;
      }
      else if(this.children.size() == 2)
      {
        if(this.children.get(0).value == this.children.get(1).value)
        {
          this.addLabel(Label.UNIVALENT);
          this.value = this.children.get(0).value;
        }
        ...
        {
          this.addLabel(Label.BIVALENT);
          this.addLabel(Label.CRITICAL);
        }
      }
    }
